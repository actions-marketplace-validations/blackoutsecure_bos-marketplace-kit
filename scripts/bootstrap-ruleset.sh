...
#
# Part of the bos-marketplace-kit toolkit. The ruleset blocks
# `.github/workflows/**` on the default branch of every covered repo
# at the GitHub platform level, complementing the in-repo `guard`
# composite (fast PR feedback) and `promote` composite (build-time
# enforcement).
#
# Usage:
...
#
# See docs/publishing.md for the full enrollment flow.
...
if [ "${PLACEHOLDER_HITS}" != "0" ]; then
  die "ruleset JSON still contains BYPASS_ACTOR_ID_PLACEHOLDER — replace with the bypass actor's integration / App ID before running. See docs/publishing.md."
fi
...
# Refuse to ship if any repo name still looks like a placeholder.
PLACEHOLDER_REPOS="$(jq -r '
  .conditions.repository_name.include[]
  | select(test("^REPLACE_ME"; "i"))
' "${RULESET_PATH}")"
if [ -n "${PLACEHOLDER_REPOS}" ]; then
  die "ruleset JSON contains placeholder repo names — replace with real repos in your org: ${PLACEHOLDER_REPOS}"
fi
