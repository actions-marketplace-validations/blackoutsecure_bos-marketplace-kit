"""Unit tests for the repo-metadata composite helper module.

The module under test is shipped *inside* the
``.github/actions/repo-metadata/`` directory so the composite stays
self-contained for external consumers. It is loaded by ``conftest.py``
under the alias ``repo_metadata_helper``.
"""
...
class TestExtractReadmeSummary:
    ...
    def test_clips_at_word_boundary_no_ellipsis(self) -> None:
        # The seed clip is for AI prompt budget; it must NOT add an
        # ellipsis (the final user-visible clamp does that).
        ...
        assert not out.endswith("...")
        # Word-boundary cut: should not end with a partial word.
...
class TestClampDescription:
    ...
    def test_clamps_with_ellipsis_at_word_boundary(self) -> None:
        ...
        assert len(out) <= 20
        assert out.endswith("…")
        # Should not have a partial trailing word (count chars before ellipsis).
        body = out.rstrip("…")
    ...
    def test_total_length_includes_ellipsis(self) -> None:
        ...
        # Even with no spaces, the cut must respect the limit.
        assert len(out) <= 50
...
class TestSanitizeTopics:
    ...
    def test_caps_at_github_ceiling_even_when_max_count_higher(self) -> None:
        ...
        assert len(out) == 20  # GitHub's per-repo ceiling
    ...
    def test_accepts_unicode_by_dropping_non_ascii(self) -> None:
        # Non-ASCII letters are not allowed in GitHub topics; they
        # should be replaced with hyphens (and possibly stripped).
